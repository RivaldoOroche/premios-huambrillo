  ...
  const handleReservar = async () => {
    ...
    try {
      const res = await fetch("/api/tickets/reservar", {
        ...
        body: JSON.stringify({ ...form, sorteo_id: sorteo.id, tipo: "aleatorio" }),
      });
            ...
            {paso === "pago" && (
              <motion.div key="pago" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {/* Número */}
                  <div className="bg-white border-2 border-[#c9a84c]/30 rounded-xl p-4 flex items-center justify-between gap-4">
                    <div>
                      <p className="text-xs text-neutral-500 uppercase tracking-widest mb-1">📱 Número</p>
                  ...
                  <div className="bg-white border-2 border-[#c9a84c]/30 rounded-xl p-4 flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-neutral-400 mt-0.5">Solo válido para PLIN</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setMostrarQR(true)}
                      className="flex items-center gap-2 bg-[#c9a84c] hover:bg-[#e0c068] text-[#1a3a2a] font-black text-xs uppercase tracking-widest px-4 py-2 rounded-xl transition-all hover:scale-105"
                    >
                      Ver QR →
                    </button>
                  </div>
