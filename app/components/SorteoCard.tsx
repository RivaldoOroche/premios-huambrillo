...
export default function SorteoCard({ sorteo }: Props) {
  return (
    <div className="bg-[#111] border-2 border-neutral-700 rounded-2xl overflow-hidden hover:border-yellow-400 hover:-translate-y-1 transition-all duration-200">
      ...
      {/* Header */}
      <div className={`px-4 py-3 flex items-center gap-3 ${sorteo.esEspecial ? "bg-yellow-500" : "bg-red-600"}`}>
        ...
        <span className={`font-black text-lg tracking-widest uppercase ${sorteo.esEspecial ? "text-black" : "text-white"}`}>
          {sorteo.fecha}
        </span>
      </div>
      ...
      {/* Body */}
      <div className="p-5">
        <h2 className={`text-2xl font-black tracking-widest uppercase mb-4 ${sorteo.esEspecial ? "text-white" : "text-yellow-400"}`}>
          {sorteo.titulo}
        </h2>
        ...
        <ul className="mb-4 space-y-2">
          {sorteo.premios.map((premio, i) => (
            <li key={i} className="flex items-center gap-3 border-b border-neutral-800 pb-2 last:border-0">
              ...
              <span className={`text-sm ${premio.esMayor ? "text-yellow-400 font-black text-base" : "text-neutral-300"}`}>
                {premio.nombre}
              </span>
            </li>
          ))}
        ...
        <div className="bg-[#1a1a1a] rounded-xl p-3 text-center mb-4">
          ...
          <p className="text-5xl font-black text-yellow-400 leading-none">
            <span className="text-2xl">S/ </span>{sorteo.precio}
          </p>
        ...
        <a
          ...
          className={`block w-full text-center py-3 rounded-xl font-black text-lg uppercase tracking-wide transition-all hover:scale-105 ${
            sorteo.esEspecial
