...
const estadoConfig: Record<string, { label: string; color: string; emoji: string }> = {
  ...
  rechazado: {
    label: "Rechazado",
    color: "bg-red-600/20 border-red-600/40 text-red-600",
    emoji: "❌",
  },
  reservado: {
    label: "Reservado",
...
export default function ConsultarTickets() {
  ...
  const buscar = async () => {
    ...
    if (!valor) {
      setError("Ingresa tu DNI o teléfono para continuar");
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/tickets/consultar?busqueda=${encodeURIComponent(valor)}`
      );

      const data = await res.json();

      if (!res.ok) {
        throw new Error(data.error || "Error al consultar tickets");
      }

      setTickets(Array.isArray(data) ? data : []);
      setBuscado(true);
    } catch (error) {
      setError(error instanceof Error ? error.message : "Error al consultar tickets");
      setTickets([]);
      setBuscado(true);
    } finally {
      setCargando(false);
  ...
  const inputClass =
    "flex-1 bg-white border-2 border-[#c9a84c]/30 rounded-xl px-4 py-3 text-[#1a1a1a] text-sm placeholder:text-neutral-400 focus:outline-none focus:border-[#c9a84c] transition-colors";
      ...
      <div className="bg-white border-2 border-[#c9a84c]/30 rounded-2xl p-6 shadow-sm space-y-3">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ej: 71053073 o 966821978"
            ...
            {tickets.length === 0 ? (
              <div className="bg-white border-2 border-[#c9a84c]/30 rounded-2xl p-8 text-center shadow-sm">
                ...
                <p className="text-neutral-600 font-bold">
                  No encontramos tickets con ese dato.
                </p>

                <p className="text-neutral-400 text-sm mt-1">
                  Verifica que el DNI o teléfono sea el mismo con el que compraste.
                </p>
              </div>
            ) : (
              <>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between flex-wrap gap-3">
                        ...
                        <span
                          className={`flex items-center gap-1.5 text-xs font-black px-3 py-1.5 rounded-full border ${cfg.color}`}
                        >
                          {cfg.emoji} {cfg.label}
                        </span>
                      </div>
